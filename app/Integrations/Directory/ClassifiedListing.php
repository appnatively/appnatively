...
class ClassifiedListing extends Provider {
    ...
    public function boot(): void {
        ...
        add_filter( "craf_appna_directory_classified-listing_categories", [$this, "categories"], 10, 3 );
        add_filter( "craf_appna_directory_classified-listing_category", [$this, "category"], 10, 3 );
    ...
    public function related_listings( ?ListingPaginatorDTO $listing_paginator, Request $request, array $fields = [] ): ListingPaginatorDTO {
        ...
        foreach ( [$this->category_taxonomy(), $this->tag_taxonomy(), $this->location_taxonomy()] as $taxonomy ) {
            ...
            if ( ! is_wp_error( $ids ) && ! empty( $ids ) ) {
                $tax_query[] = ["taxonomy" => $taxonomy, "field" => "term_id", "terms" => array_map( "intval", $ids )];
            }
        }
    ...
    private function map_listing_to_dto( WP_Post $post, array $fields ): ListingDTO {
        ...
        if ( in_array( "address", $fields, true ) ) {
            $dto->set_address( $this->get_meta_value( $post->ID, ["address", "_address", "rtcl_address"] ) );
        }
        if ( in_array( "latitude", $fields, true ) ) {
            $dto->set_latitude( $this->normalize_coordinate( $this->get_meta_value( $post->ID, ["latitude", "_latitude", "rtcl_latitude"] ), -90, 90 ) );
        }
        if ( in_array( "longitude", $fields, true ) ) {
            $dto->set_longitude( $this->normalize_coordinate( $this->get_meta_value( $post->ID, ["longitude", "_longitude", "rtcl_longitude"] ), -180, 180 ) );
        }
        if ( in_array( "phone", $fields, true ) ) {
            $dto->set_phone( $this->get_meta_value( $post->ID, ["phone", "_phone", "rtcl_phone"] ) );
        }
        if ( in_array( "email", $fields, true ) ) {
            $dto->set_email( $this->get_meta_value( $post->ID, ["email", "_email", "rtcl_email"] ) );
        }
        if ( in_array( "website", $fields, true ) ) {
            $dto->set_website( $this->get_meta_value( $post->ID, ["website", "_website", "rtcl_website"] ) );
        }
        if ( in_array( "favorite", $fields, true ) ) {
            $dto->set_favorite( false );
        }
        if ( in_array( "featured", $fields, true ) ) {
            $dto->set_featured( (bool) $this->get_meta_value( $post->ID, ["featured", "_featured", "is_featured"] ) );
        }
        if ( in_array( "new", $fields, true ) ) {
            $dto->set_new( (bool) $this->get_meta_value( $post->ID, ["_rtcl_is_new", "is_new"] ) );
        }
        if ( in_array( "popular", $fields, true ) ) {
            $dto->set_popular( false );
        }
        if ( in_array( "pricing", $fields, true ) ) {
            $dto->set_pricing( ["price" => $this->get_meta_value( $post->ID, ["price", "_price"] ), "price_type" => $this->get_meta_value( $post->ID, ["price_type", "_price_type", "pricing_type"] ), "price_range" => $this->get_meta_value( $post->ID, ["price_range", "_price_range"] )] );
        }
    ...
    private function get_terms_page( string $taxonomy, Request $request, int $page, int $per_page ): array {
        ...
        $args  = ["taxonomy" => $taxonomy, "hide_empty" => false, "number" => $per_page, "offset" => ( $page - 1 ) * $per_page, "search" => sanitize_text_field( (string) $request->get_param( "search" ) ), "orderby" => $map[$key] ?? "name", "order" => $desc ? "DESC" : "ASC"];
        $items = get_terms( $args );
        $total = wp_count_terms( ["taxonomy" => $taxonomy, "hide_empty" => false, "search" => $args["search"]] );
        return ["items" => is_wp_error( $items ) ? [] : $items, "total" => is_wp_error( $total ) ? 0 : (int) $total, "last_page" => max( 1, (int) ceil( ( is_wp_error( $total ) ? 0 : (int) $total ) / $per_page ) )];
    }
    ...
    private function get_term_image( int $term_id ): array {
        foreach ( ["rtcl_image", "image", "thumbnail_id", "_thumbnail_id"] as $key ) {
    ...
    private function query_comment_reviews( int $listing_id, Request $request ): array {
        $base         = ["post_id" => $listing_id, "status" => "approve"];
        ...
        foreach ( $query["rows"] as $row ) {
            $comment = $row["comment"];
            $items[] = ["id" => (int) $comment->comment_ID, "reviewer" => sanitize_text_field( (string) $comment->comment_author ), "review" => wp_kses_post( (string) $comment->comment_content ), "rating" => $row["rating"], "date_created" => (string) get_comment_date( DATE_ATOM, $comment ), "avatar_url" => (string) get_avatar_url( $comment, ["size" => 96] )];
        }
    ...
    private function get_comment_rating( int $comment_id ): float {
        foreach ( ["rating", "_rating", "rtcl_rating"] as $key ) {
